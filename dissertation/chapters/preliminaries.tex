...
Another useful theorem which we will use is the Sobolev imbedding theorem, we only care for case 3 from Theorem 1.44 from Dacorogna \cite{dacorogna}.
\begin{theorem}[Sobolev embedding theorem]
\label{thm:embedding}
Let $\Omega \subset \mathbb{R}^2$ be a bounded open set with Lipschitz boundary.
\begin{enumerate}
    \item [Case 3.] If $p>n$ then
    \[
    W^{1,p}(\Omega) \subset C^{0,\alpha}(\overline{\Omega}), \quad \text{for every } \alpha \in [o,1-n/p].
    \]
\end{enumerate}
\end{theorem}
