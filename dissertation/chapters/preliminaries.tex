...
\]
The derivation for this formulation can be found at the appendix (yet to write). The integrand is given as 
\[
...
We now extend the Euler--Lagrange Theorem~\ref{thm:Euler-Lagrange} in the setting of the Direct Method, motivated by the general existence theorem, Theorem~\ref{thm:General_existence_theorem}. We then extend the result to minimisers in the setting of $\overline{u} \in W^{1,p}(\Omega)$.
...
We now have all the tools we need to use the direct method for the model terrain problem. {\color{red} cite this after introis written}