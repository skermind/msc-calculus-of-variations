...
As mentioned previously Didos problem concerns ourselves with finding curve that encloses the maximum area with a given restriction on length of said curve. We can formulate this problem in the following way. 

Among the smooth functions $y$ which are positive on $(0,l)$ and which vanish at endpoints $x=0$ and $x=l$, find the function that gives a minimum for
\[
J(y) = -\int_0^ly(x)\sqrt{1+(y'(x))^2} \, dx
\]
...
\begin{align*}
 f-y' f_{y'} &\iff -y\sqrt{1-y'^2} -y' \left(\frac{yy'}{\sqrt{1-y'^2}}\right) = c\\
...
\[
(x(s),y(s)) = (-\frac{l}{\pi}\cos(\frac{\pi}{l}s) - \frac{l}{\pi}, \frac{l}{\pi}\sin(\frac{\pi}{l}s))
\]
